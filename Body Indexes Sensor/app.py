...
def health_metric(metric):
    if 'user_id' in session:
        ...
        # Initialize a dictionary to hold the fetched data
        metric_data_queries = {
            'HeartRate': 'SELECT timestamp, heart_rate FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'BloodPressure': 'SELECT timestamp, blood_pressure FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'BodyTemp': 'SELECT timestamp, body_temp FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'BloodOxygenLevel': 'SELECT timestamp, blood_oxygen_level FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'RespiratoryRate': 'SELECT timestamp, respiratory_rate FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'BloodSugarLevel': 'SELECT timestamp, blood_sugar_level FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
            'SleepQuality': 'SELECT timestamp, sleep_quality FROM health WHERE user_id = %s ORDER BY timestamp DESC LIMIT 7',
        }

        metric_data = []
        if metric in metric_data_queries:
            query = metric_data_queries[metric]
            cur.execute(query, (user_id,))
            metric_data = cur.fetchall()
        ...
        if health_data:
            ...
            # Ensure metric_data is not None
            if metric_data is None:
                metric_data = []

            return render_template('main.html', name=name, metric=metric, metric_value=metrics.get(metric, 'No data available'), metric_data=metric_data)
        else:
            return redirect(url_for('dashboard'))
    else:
        return redirect(url_for('login'))
